# Computing progression — full Teach Computing detail, by stage (year) and strand

> **Why this exists.** Your school marking system's *ICT Stages* are a heavily cut-down summary of the
> **Teach Computing Curriculum** (NCCE / Raspberry Pi Foundation), so the lower stages especially carry
> almost no detail. This document pulls the **full per-year curriculum** — every learning objective /
> lesson and its **"I can…" success criteria** — laid out **by stage (one stage = one year) then by
> strand**, to seed the fine-grained [Stages & strands](PHASE_16_PLAN.md) level system. This is the
> detail you were expecting; [LEVEL_SYSTEM_MISSING_DESCRIPTORS.md](LEVEL_SYSTEM_MISSING_DESCRIPTORS.md)
> only diffed the high-level National Curriculum statements, so it looked far too sparse.
>
> **Source:** `docs/TeachComputing_docs/` curriculum-map workbooks. Teach Computing Curriculum
> © Raspberry Pi Foundation, used under its open terms. **No pupil data** is involved.

## Mapping used (you chose: one stage = one year)

| Stage | Maps to | Key stage |
|---|---|---|
| **Stage 6** | Year 1 · age 5–6 | KS1 |
| **Stage 7** | Year 2 · age 6–7 | KS1 |
| **Stage 8** | Year 3 · age 7–8 | KS2 |
| **Stage 9** | Year 4 · age 8–9 | KS2 |
| **Stage 10** | Year 5 · age 9–10 | KS2 |
| **Stage 11** | Year 6 · age 10–11 | KS2 |
| **Stage 12** | Year 7 · age 11–12 | KS3 |
| **Stage 13** | Year 8 · age 12–13 | KS3 |
| **Stage 14** | Year 9 · age 13–14 | KS3 |
| Stages 1–5 | EYFS / Reception & below | — (pre-NC) |

> **⚠️ This shifts the key-stage boundaries** versus the first missing-descriptors doc (which had Stages
> 6–8 = KS1, 9–11 = KS2). With one-year-per-stage: **KS1 = Stages 6–7**, **KS2 = Stages 8–11**, **KS3 =
> Stages 12–14**. Age alignment was your priority; shift the numbers if your stages start a year
> earlier/later.

## How to read each stage

- Each stage = one school year, grouped by **Teach Computing strand**.
- **KS1–KS2 (Stages 6–11):** each **unit → learning objective** (the lesson aim) → its **"I can…"**
  success criteria.
- **KS3 (Stages 12–14):** the curriculum lists criteria directly per lesson, so it's **unit → lesson →**
  the criteria for that lesson (no separate one-line aim).
- In the Stages & strands model: a learning objective / lesson ≈ a *criterion group*, each **"I can…"**
  ≈ a *criterion* you tick. Strands present vary by year.
...
*36 lessons · 98 "I can…" success criteria across 6 units.*
...
*36 lessons · 97 "I can…" success criteria across 6 units.*
...
*36 lessons · 96 "I can…" success criteria across 6 units.*
...
Across Stages 6–14: **324 learning objectives / lessons** and **939 "I can…" success criteria** —
versus the **120** descriptors in the current school system. The early stages gain the most: where the
school had 1–2 descriptors at Stages 6–8, the full curriculum has ~36 objectives / ~108 criteria each.

## Notes for seeding

- **Pick your grain.** You needn't import all ~830 "I can…" lines. A practical middle path: seed the
  **learning objectives / lessons** as criteria (≈36/year), with the "I can…" lines as the evidence
  checklist behind each.
- **Strands.** Teach Computing uses 10 strands; your confirmed scheme uses fewer. Map them when seeding,
  e.g. **PG+AL → Programming**, **CS+NW → Computer systems & networks**, **SS+IT → Online safety**,
  **DI → Data**, **CM+DD+ET → Creating/IT**.
- **EYFS (Stages 1–5)** isn't covered (Teach Computing primary starts at Year 1); keep your existing
  early-years statements or source EYFS 'Understanding the World – Technology' separately.
- **KS4/GCSE** is handled by the separate OCR-J277 grade scheme, not this ladder.
