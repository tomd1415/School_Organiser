...
| **8.8** | *(stretch)* auto-marked question types (tick/multiple-choice keyed from the answers doc), teacher comment back to pupil, per-pupil "what works for me" profile from feedback history, printable class answer pack, kiosk auto-login per room | nice-to-haves | L |
