...
export function registerUiGalleryRoutes(app: FastifyInstance): void {
  app.get('/ui-gallery', { preHandler: requireAuth }, async (_req, reply) => {
    ...
    const body = `<section class="card workspace-width">
      ...
      ${item('Now — hero', 'renderNowHero — the prominent “what’s happening now” strip atop the Now screen: period eyebrow, lesson title, room + start time, the time-remaining countdown, and what’s next. On the live page it self-polls every 30s; the gallery renders it inert (poll=false) so it never swaps live data over this fixture.', renderNowHero(GALLERY_NOW_HERO_STATE, GALLERY_NOW_HERO_LESSON, GALLERY_NOW_HERO_NEXT, undefined, undefined, false))}
