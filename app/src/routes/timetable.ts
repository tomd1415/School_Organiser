...
function renderLesson(l: GridLesson, date: string): string {
  ...
  const inner = `<span class="tt-group">${flag}${heading}</span>${courses ? `<span class="tt-course">${courses}</span>` : ''}`;
...
function renderCell(cell: GridCell, date: string, isToday: boolean): string {
  ...
  return `${td}${cell.lessons.map((l) => renderLesson(l, date)).join('')}</td>`;
}
  ...
  app.get('/timetable', { preHandler: requireAuth }, async (req, reply) => {
    ...
    try {
      ...
      const head = `<tr><th class="tt-corner"></th>${weekDates
        .map((d, i) => `<th${d === today ? ' class="tt-today"' : ''}>${esc(DAY_NAMES[i] ?? '')}<span class="tt-date">${esc(fmtShort(d))}</span></th>`)
        .join('')}</tr>`;
        ...
        .map((row) => {
          const cells = row.cells
            .map((cell, i) => {
              ...
              return renderCell(cell, d, d === today);
            })
            .join('');
