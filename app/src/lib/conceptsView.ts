...
export function renderRow(c: ConceptRow, courses: { id: number; name: string }[]): string {
  const save = (field: string) =>
    `hx-post="${paths.concept(c.id)}" hx-vals='{"field":"${field}"}' hx-trigger="input changed delay:700ms, blur" hx-swap="none"`;
  return `<tr class="concept-row${c.active ? '' : ' kit-archived'}" id="concept-${c.id}">
    ...
    <td><select name="value" hx-post="${paths.conceptCourse(c.id)}" hx-trigger="change" hx-swap="none">${courseOptions(courses, c.courseId)}</select></td>
    ...
    <td>
      ...
      ${c.active
        ? `<button type="button" class="link danger" hx-post="${paths.conceptArchive(c.id)}" hx-target="#concept-${c.id}" hx-swap="outerHTML" hx-confirm="Archive this concept? It stays in the records but stops being woven into lessons.">archive</button>`
        : `<button type="button" class="link" hx-post="${paths.conceptRestore(c.id)}" hx-target="#concept-${c.id}" hx-swap="outerHTML">restore</button>`}
    </td>
  </tr>`;
}
...
export function renderConceptsNext(data: ConceptsNextData): string {
  ...
  return `
    <section class="card concepts-overhaul" hx-headers='{"x-csrf-token":"${csrf}"}'>
      ...
      ${table}
      <form class="kit-add" hx-post="${paths.conceptAdd()}" hx-target="closest section" hx-swap="outerHTML">
