...
- use a button press to change a variable
- use movement (the accelerometer) to change a variable
- show a variable, and know that checking it does not change its value
...
> 🧑‍🏫 On the starter worksheet pupils tick all the inputs. The LED screen is the odd one out — it is an output. Support pupils choose what an input is.

## A variable counts
- a variable holds a value that can change
- set the count to 0 at the start
- change the count by 1 to count up
...
> 🧑‍🏫 Likely error: the count jumps up a lot on a shake — many tiny movements each change it. Note this; it is fixed in Lesson 6.

## Checking a variable doesn't change it
- showing the count just looks at the value
- the value stays the same after it is checked

> 🧑‍🏫 Show two values of 8 in a row when shaken but not pressed — proof that checking does not change the value. Core pupils choose "stays the same"; Challenge explain why.

## I can…
Tick your three "I can…". Tell me one input you used.

> 🧑‍🏫 Listen for button or shake/accelerometer. Movement break before the next lesson.
