# ScratchJr recap

## Today we are learning
- find the parts of the ScratchJr screen
- find the start of a sequence
- see that a program needs to be started
- run my program

> 🧑‍🏫 Read the four "I can…" aloud. Vocabulary on the board: sequence · command · program · run · start. Same routine as always. No sound or animation needed on the board.

## The ScratchJr screen
![The ScratchJr screen]({{res:l1-scratchjr-screen.png}})

- the **stage** is where the sprite moves
- the **blocks** make the program
- the **green flag** starts it

> 🧑‍🏫 Point to each part. Ask pupils to point to the same part on their own screen. The worksheet has a LABEL task for this.

## A sequence has a start
- a **sequence** is things that happen in order
- the school day: gate opens → line up → walk to class
- the **first** thing is the start

> 🧑‍🏫 Use your own school routine. Likely error: pupils name a later step as the start → fix-words "the start is the very first thing."

## Start the program
![The yellow start blocks]({{res:l1-start-blocks.png}})

- a program does not move until you start it
- press the **green flag** to start
- this is the same as the start of a sequence

> 🧑‍🏫 I-do then we-do. Build a tiny program and show it does nothing until the green flag is pressed.

## Run it full screen
![A program running full screen]({{res:l1-fullscreen.png}})

- press the green flag
- watch your sprite move
- press it again to run it again

> 🧑‍🏫 You-do. Pupils run their own program and screenshot it. Support: pair with a confident partner. Challenge: make it run twice.

## I can…
Tick your four "I can…". Show me your green flag.

> 🧑‍🏫 Note who can start a program on their own. Next lesson: what a program makes happen (the outcome).
