...
- name what `src` and `alt` do
- build a web page to match a design
...
> 🧑‍🏫 Recap from last lesson. Stress: a closing tag starts with `</`. Mention `<em>`/`<strong>` help screen readers.

## The img tag  (I do)
`<img src="dog.jpg" alt="My dog">`

- `src` = the file name of the image
- `alt` = a description, read out by screen readers or shown if the image fails
...
> 🧑‍🏫 Support: card-sort tags vs attributes first. Core: order the steps to add an image. Challenge: write the whole tag. Paste code + screenshot on the activity worksheet.

## I can…
Tick your three "I can…". Show me your page with an image.

> 🧑‍🏫 Note who set both `src` and `alt` correctly.
