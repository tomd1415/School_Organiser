...
test('UI gallery renders every showcased component with no client-side errors', async ({ page }) => {
  ...
  // each section renders (component kit + 14 fixture-backed views)
  await expect(page.locator('.gallery-item')).toHaveCount(15);
  ...
  await expect(page.locator('.gallery-stage .cov-area').first()).toBeVisible(); // Coverage area cards
